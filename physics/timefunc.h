...
timefunc.h
	...
	This class contains parameters for time-dependent operation of
   boundaries.  It can be used in any place where time parameterization
	is needed.

0.95  (PeterM 3/8/95) Creation	
0.97  (PeterM 6/11/96) Added string parser using strings
...
class TimeFunction 
{
  ...
  int time_function_flag; /* selects from any various interpretations of above */
 ...
 public:
  TimeFunction(Scalar _A, Scalar _C, Scalar _f, Scalar _p, Scalar _tr,
               Scalar _tf, Scalar _tp, Scalar _td, Scalar _a0, Scalar _a1, 
               ostring F,int _tflag=0) 
    {
      A=_A; C = _C; w = _f*TWOPI; phase = _p; trise = _tr;
      tfall = _tf; tpulse = _tp; tdelay = _td;
      a0 = _a0; a1 = _a1; time_function_flag = _tflag;
      Func = "0";
      evaluator = new Evaluator(adv_eval);
      if(time_function_flag==1) {
        Func=F+ (ostring)'\n';
