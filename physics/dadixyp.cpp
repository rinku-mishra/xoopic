...
*/
  ...
  for(i=0; i<ng1-1; i++) // subtract 1, the last boundary is in fact the same as the 0'th
  {
    for(j=0; j<ng2; j++)
    {
     ...
     /*  The following code handles some special cases like corners*/
	  for(j=0; j<ng2; j++)
		  if(b_x2geom[i][j]!=0)  //non-dirichlet
			  r_x2[j] = uadi[i][j] +dth*(-s[i][j] 
													+((i>0)?a_x1geom[i][j]*uadi[i-1][j]:a_x1geom[i][j]*uadi[nc1-1][j])
													+b_x1geom[i][j]*uadi[i][j] 
													+((i!=nc1-1)?c_x1geom[i][j]*uadi[i+1][j]:c_x1geom[i][j]*uadi[0][j]));
		  else
			  r_x2[j] = uadi[i][j];  // metal sets the potential here.
          
    /* Solve tridiagonal system. */
    tridag(a_x2, b_x2, c_x2, r_x2, v_x2, gam_x2, ng2);
    /* Copy solution into ustar. */
    for(j=0; j<ng2; j++) ustar[i][j] =v_x2[j];

  }
//pvs fix for equipotential endpoint problem:
  for(j=0;j<=nc2;j++) ustar[nc1][j]=ustar[0][j];//=0.125*(uadi[1][j]+uadi[nc1-1][j]+4*uadi[0][j]+uadi[0][j+1]+uadi[0][j-1]);
  //uadi[nc1][0]=uadi[0][0]=0.166666667 * (uadi[1][0]+uadi[nc1-1][0]+3*uadi[0][0]+uadi[0][1]);
  //uadi[nc1][nc2]=uadi[0][nc2]=0.166666667 * (uadi[1][nc2]+uadi[nc1-1][nc2]+3*uadi[0][nc2]+uadi[0][nc2-1]);
  /***************************************/
  ...
  /*  handle the boundary conditions, neumann and dirichlet */
	  for(i=0; i<ng1; i++)
		  if(b_x2geom[i][j]!=0&&i!=0&&i!=nc1)  //non-dirichlet, 0 and nc1 treated as dirichlet
			  r_x1[i] = ustar[i][j] +dth*(-s[i][j] 
													+((j>0)?a_x2geom[i][j]*ustar[i][j-1]:0)
													+b_x2geom[i][j]*ustar[i][j] 
													+((j<nc2)?c_x2geom[i][j]*ustar[i][j+1]:0));
		  else
			  r_x1[i] = ustar[i][j];  // metal sets the potential here.
	 //set the coefficients as if the edges were conductors
	 a_x1[0]=a_x1[nc1]=c_x1[0]=c_x1[nc1]=0;
	 b_x1[0]=b_x1[nc1]=1;
    /* Solve tridiagonal system. */
    tridag(a_x1, b_x1, c_x1, r_x1, v_x1, gam_x1, ng1);

    /* Copy solution into ustar. */
    for(i=0; i<ng1; i++) uadi[i][j] =v_x1[i];
  }
		 ...
		 case FREESPACE:
			{
				...
				if(i==0) a_x1geom[i][j]=c_x1geom[i][j];
