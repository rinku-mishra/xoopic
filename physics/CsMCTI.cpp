...
//
// Tunneling ionization of Cs for only two of its electrons,
// the outer most ones, is implemented and thus Ztot = 2.  
// The ionization is from the ground states. 
//  
int const CsMCTI::Ztot = 2; 
                            

CsMCTI::CsMCTI(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
  ...
  : MCTIPackage(Cs, (ostring)"Cs", p, temp, eSpecies, iSpecies, sList, dt, 
                ionzFraction, ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, 
                _Min2MKS, _Max2MKS, _delayTime, _stopTime, analyticF, 
                numMacroParticlesPerCell, argEfieldFrequency, 
                discardDumpFileNGDDataFlag){

  cout << endl << endl << "Creating CsMCTI" << endl;
  ...
  E0    = new Scalar[Ztot]; // auxiliary parameters 
  ...
  Ei[0] = 3.8939/ATOMIC_ENERGY_UNIT_EV;
  Ei[1] = 25.1/ATOMIC_ENERGY_UNIT_EV;
  ...
  for ( int i = 0; i < Ztot; i++ ) {
    ...
    wCircularFactor[i] *= pow((Scalar)(2.0*E0[i]), (Scalar)(2.0*nStar[i]-1.0));
    if ( i == 1 ) // this is needed for l = 1, m = -1 for Cs+ -> Cs++
      wCircularFactor[1] *= (3./(2.*E0[i]));
    wLinearFactor[i]    = wCircularFactor[i]*sqrt(3.0/(M_PI*E0[i]));
    ...
    if ( i == 0 ) {
      if ( (2.0*nStar[i]-1.) > 1. ) {
        // 
        // There is an extremum point, so set the Emax to it. 
        // The max field should really be determined from
        // solving the time dependent Schrodinger equation. 
        // This needs further work...!
        // 
        EmaxTI[i] = 2.0*E0[i]/(3.0*(2.0*nStar[i]-1.)); 
      } else {
        stringstream ss (stringstream::in | stringstream::out);
        ss << "CsMCTI::CsMCTI: Error: \n" << 
        ...
        ss >> msg;
        Oops oops(msg);
        throw oops;    // exit()CreatecounterPart.
      }
    } else if ( i == 1 ) {
      if ( (2.0*nStar[i]-1.) > 1. ) {
        // 
        ...
        // This needs further work...!
        // 
        EmaxTI[i] = 2.0*E0[i]/(3.0*(2.0*nStar[i]-2.)); 
      } else {
        stringstream ss (stringstream::in | stringstream::out);
        ss << "CsMCTI::CsMCTI: Error: \n" << 
           "WARNING: The tunneling ionziation probability rate " << endl
             << "does not have an extremum for the ionization of Cs." << endl
             << "The max field for which the tunneling ionization "   << endl
             << "formula is applicable and the ionization rate for "  << endl
             << "larger fields is to be determined more precisely..." << endl;
    ...
    try {
      ...
      oops.prepend("CsMCTI::CsMCTI: Error:\n"); // OK
      throw oops;
    }
 ...
 */
void CsMCTI::tunnelIonize(ParticleGroupList** pgList, ParticleList& pList) 
  /* throw(Oops) */{
  // 
  // attempt to tunnel ionize first Cs atoms for the current values of the 
  // E field: 
  // 
    try{
      tunnelIonizeCs(pList);
    }
    catch(Oops& oops2){
      oops2.prepend("CsMCTI::tunnelIonize: Error: \n");//done
      throw oops2;
    }

  // 
  // find the ParticleGroupList for the CsPlus species (ions) and then
  ...
  for (siter.restart(); !siter.Done(); siter++) {
    ...
    if ( i == iSpecies->getID() ) {
      // 
      // now attempt to tunnel ionize the CsPlus ions
      // 
      try{
        tunnelIonizeCsPlus(*pgList[i], pList); 
      }
      catch(Oops& oops3){
        oops3.prepend("CsMCTI::tunnelIonize: Error: \n"); //done
        throw oops3;
      }
...
void CsMCTI::tunnelIonizeCs(ParticleList& pList) 
  /* throw(Oops) */{
  /**
